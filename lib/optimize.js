...
function doOptimizeReduce(read, reduce, graph, optimization_info) {
...
let optimizer = {
    optimize_reduce: function(read, reduce, graph, optimization_info) {
        doOptimizeReduce(read, reduce, graph, optimization_info);

        if (optimization_info.invalid) {
            // XXX this would be good to move into the core juttle errors
            throw new errors.CompileError(`read googleanalytics does not support ${optimization_info.invalid}`,
                'ADAPTER-UNSUPPORTED-FEATURE',
                {
                    location: reduce.location
                }
            );
        }
