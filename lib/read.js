...
class ReadGoogleAnalytics extends AdapterRead {
    constructor(options, params) {
        ...
        if (this.options.from) {
            if (! this.options.from.quantize(JuttleMoment.duration(1, 'd')).eq(this.options.from)) {
        ...
        this.now = params.now;
        if (this.options.to) {
            if (! this.options.to.eq(params.now) &&
                ! this.options.to.quantize(JuttleMoment.duration(1, 'd')).eq(this.options.to)) {
                throw errors.compileError('ADAPTER-UNSUPPORTED-TIME-OPTION', {
                    option: '-to',
