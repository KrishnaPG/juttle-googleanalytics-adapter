...
function toJuttle(results, extras) {
    ...
    for (let row = 0; row < data.length; ++row) {
        ...
        for (let col = 0; col < columns.length; ++col) {
            ...
            // The way the API exposes the time dimensions is a bit odd... for
            // groupings by week/month/minute, the API takes two dimensions -- a
            // base value (e.g. year) and an offset within that base(e.g. month).
            //
            // These come back in a predictable order, so to compute the time on
            // the point, parse the base value and then for the offset field,
            // add an appropriate shift to the previously computed base time.
            switch (name) {
                case 'year':
